...
function B = bfilter2(A,w,sigma,J,sigma_n)
...
if size(A,3) == 1
   B = bfltGray(A,w,sigma(1),sigma(2),J,sigma_n);
else
...
function B = bfltGray(A,w,sigma_d,sigma_r,J,sigma_n)
...
for i = 1:dim(1)
   for j = 1:dim(2)
         ...
         H = exp(-max((I-J(i,j)).^2-sigma_n^2,0)/(2*sigma_r^2));
